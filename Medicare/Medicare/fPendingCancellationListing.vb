Public Class fPendingCancellationListing
    Dim _objBusi As New Business
    Dim Conn As DbConnection = New SqlConnection
    Private Sub fPendingCancellationListing_Disposed(ByVal sender As Object, ByVal e As System.EventArgs) Handles Me.Disposed
        If Conn.State = ConnectionState.Open Then
            Conn.Close()
        End If
    End Sub
    Private Sub fPendingCancellationListing_Load(ByVal sender As Object, ByVal e As System.EventArgs) Handles Me.Load
...
#End Region
    Private Sub BINDDATA()
        ...
        StartMarqueeThread()
        ds = _objBusi.getDsDetails_V("PCL", "", "", "", "", "", "", "", "", "", "", Conn)
        SyncLock SharedData
            SharedData.ReadyToHideMarquee = True
        End SyncLock
        Application.DoEvents()
        If ds.Tables(0).Rows.Count > 0 Then
            With Me.dgvPendingCancellationListing
            ...
            End With
        Else
            Me.dgvPendingCancellationListing.DataSource = ds.Tables(0)
            Me.tslblTOT.Text = "Total Record : 0 "
        End If
    End Sub
    Private Sub dgvPendingCancellationListing_CellContentClick(ByVal sender As System.Object, ByVal e As System.Windows.Forms.DataGridViewCellEventArgs) Handles dgvPendingCancellationListing.CellContentClick
        With Me.dgvPendingCancellationListing
            If e.ColumnIndex = 3 Then
                ...
                If Not IsDBNull(.Rows(e.RowIndex).Cells(4).Value) Then
                    Dim sRes As String
                    sRes = _objBusi.spUpdate("INSERTPP", .Rows(e.RowIndex).Cells(4).Value.ToString.Trim(), "NONPAYMENT", .Rows(e.RowIndex).Cells(0).Value.ToString.Trim(), .Rows(e.RowIndex).Cells(1).Value.ToString.Trim(), .Rows(e.RowIndex).Cells(2).Value.ToString.Trim(), "", My.Settings.Username.ToUpper.ToString.Trim(), "", "", "NONPAYMENT", Conn)
                    _objBusi.spUpdate("UPDATEPP", .Rows(e.RowIndex).Cells(5).Value.ToString.Trim(), "NONPAYMENT", .Rows(e.RowIndex).Cells(0).Value.ToString.Trim(), .Rows(e.RowIndex).Cells(1).Value.ToString.Trim(), .Rows(e.RowIndex).Cells(1).Value.ToString.Trim(), "", My.Settings.Username.ToUpper.ToString.Trim(), "", "", "NONPAYMENT", Conn)
                    If sRes = "1" Then
        ...
        End With
    End Sub
    Private Sub dgvPendingCancellationListing_CellDoubleClick(ByVal sender As Object, ByVal e As System.Windows.Forms.DataGridViewCellEventArgs) Handles dgvPendingCancellationListing.CellDoubleClick
        If Me.dgvPendingCancellationListing.SelectedRows.Count > 0 Then
            With Me.dgvPendingCancellationListing
    ...
    Private Sub Xport2Xcel()
        If Me.dgvPendingCancellationListing.Rows.Count > 0 Then
            SharedData.ReadyToHideMarquee = False
            StartMarqueeThread()
            ...
            With xWB.Worksheets(xWName)
                .Cells(1, 1) = "PENDING CANCELLATION LISTING DETAILS"
                ...
                Do While iCount < Me.dgvPendingCancellationListing.RowCount
                    .Cells(xRowCount, 1) = "'" & (xRowCount - 5).ToString.Trim
                    .Cells(xRowCount, 2) = "'" & Me.dgvPendingCancellationListing.Rows(iCount).Cells(5).Value.ToString.Trim
                    .Cells(xRowCount, 3) = "'" & Me.dgvPendingCancellationListing.Rows(iCount).Cells(6).Value.ToString.Trim
                    .Cells(xRowCount, 4) = "'" & Me.dgvPendingCancellationListing.Rows(iCount).Cells(7).Value.ToString.Trim
                    .Cells(xRowCount, 5) = "'" & Me.dgvPendingCancellationListing.Rows(iCount).Cells(8).Value.ToString.Trim
                    .Cells(xRowCount, 6) = "'" & Me.dgvPendingCancellationListing.Rows(iCount).Cells(9).Value.ToString.Trim
                    .Cells(xRowCount, 7) = "'" & Me.dgvPendingCancellationListing.Rows(iCount).Cells(10).Value.ToString.Trim
                    .Cells(xRowCount, 8) = "'" & Me.dgvPendingCancellationListing.Rows(iCount).Cells(11).Value.ToString.Trim
                    .Cells(xRowCount, 9) = "'" & Me.dgvPendingCancellationListing.Rows(iCount).Cells(12).Value.ToString.Trim
                    .Cells(xRowCount, 10) = "'" & Me.dgvPendingCancellationListing.Rows(iCount).Cells(13).Value.ToString.Trim
                    .Cells(xRowCount, 11) = "'" & Me.dgvPendingCancellationListing.Rows(iCount).Cells(14).Value.ToString.Trim
                    .Cells(xRowCount, 12) = "'" & Me.dgvPendingCancellationListing.Rows(iCount).Cells(15).Value.ToString.Trim
                    .Cells(xRowCount, 13) = "'" & Me.dgvPendingCancellationListing.Rows(iCount).Cells(16).Value.ToString.Trim
                    .Cells(xRowCount, 14) = ""
                    xRowCount += 1
                    iCount += 1
                Loop
