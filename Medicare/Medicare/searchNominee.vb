Public Class searchNominee
    ...
    Private Sub searchNominee_Disposed(ByVal sender As Object, ByVal e As System.EventArgs) Handles Me.Disposed
        If Conn.State = ConnectionState.Open Then
            Conn.Close()
        End If
    End Sub
    Private Sub searchNominee_Load(ByVal sender As Object, ByVal e As System.EventArgs) Handles Me.Load
    ...
    Private Sub dgvPolicyDetails_CellDoubleClick(ByVal sender As Object, ByVal e As System.Windows.Forms.DataGridViewCellEventArgs) Handles dgvPolicyDetails.CellDoubleClick
        With Me.dgvPolicyDetails
            ...
            Dim cmdSelect As SqlCommand = CType(Conn.CreateCommand(), SqlCommand)
            cmdSelect.CommandType = CommandType.Text
            cmdSelect.CommandText = "SELECT ID, Member_Policy_ID, Full_Name, Relationship, Share " & _
                                     "FROM dt_Member_Policy_Nomination WHERE Member_Policy_ID = '" & .CurrentRow.Cells(0).Value.ToString & "'"
            Dim sdp As New SqlDataAdapter(cmdSelect)
            Dim ds As New DataSet
            sdp.Fill(ds, "dt_Member_Policy_Nomination")
            With Me.dgvNomineeDetails
                .DataSource = ds
                ...
                .Columns(3).HeaderText = "Relationship"
                .Columns(4).HeaderText = "Share"
        ...
        End With
    End Sub
    Private Sub dgvNomineeDetails_CellDoubleClick(ByVal sender As Object, ByVal e As System.Windows.Forms.DataGridViewCellEventArgs) Handles dgvNomineeDetails.CellDoubleClick
        With Me.dgvNomineeDetails
            If .Rows.Count = 0 Then
                Exit Sub
            End If
            ...
            Nominee.lblNomineeID.Text = .CurrentRow.Cells(0).Value.ToString
            Nominee.lblMemberID.Text = dgvPolicyDetails.CurrentRow.Cells(1).Value.ToString
            Nominee.lblGUID.Text = ""
            Nominee.lblID.Text = .CurrentRow.Cells(1).Value.ToString
            Nominee.Show()
        End With
    End Sub
End Class