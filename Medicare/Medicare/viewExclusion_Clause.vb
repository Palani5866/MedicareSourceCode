Public Class viewExclusion_Clause
    Dim Conn As DbConnection = New SqlConnection
    Dim ds_Member As New DataSet
    Private Sub viewExclusion_Clause_Disposed(ByVal sender As Object, ByVal e As System.EventArgs) Handles Me.Disposed
        If Conn.State = ConnectionState.Open Then
            Conn.Close()
        End If
    End Sub
    Private Sub viewExclusion_Clause_Load(ByVal sender As Object, ByVal e As System.EventArgs) Handles Me.Load
    ...
    Private Sub dgvPolicyDetails_CellDoubleClick(ByVal sender As Object, ByVal e As System.Windows.Forms.DataGridViewCellEventArgs) Handles dgvPolicyDetails.CellDoubleClick
        With Me.dgvPolicyDetails
            ...
            Dim cmdSelect As SqlCommand = CType(Conn.CreateCommand(), SqlCommand)
            cmdSelect.CommandType = CommandType.Text
            cmdSelect.CommandText = "SELECT ID, Member_Policy_ID, ExclusionClause_Code, ExclusionClause_Description  " & _
                                     "FROM dt_Member_Policy_Exclusion_Clause WHERE Member_Policy_ID = '" & .CurrentRow.Cells(0).Value.ToString & "'"
            Dim sdp As New SqlDataAdapter(cmdSelect)
            Dim ds As New DataSet
            sdp.Fill(ds, "dt_Member_Policy_Exclusion_Clause")
            With Me.dgvExclusionClauseDetails
                .DataSource = ds
                ...
                .Columns(3).HeaderText = "Exclusion Clause Description"
        ...
        End With
    End Sub
    Private Sub dgvExclusionClauseDetails_CellDoubleClick(ByVal sender As Object, ByVal e As System.Windows.Forms.DataGridViewCellEventArgs) Handles dgvExclusionClauseDetails.CellDoubleClick
        With Me.dgvExclusionClauseDetails
            If .Rows.Count = 0 Then
                Exit Sub
            End If
            Dim ExclusionClause As New Exclusion_Clause
            If Me.lblAdmin.Text = "Admin" Then
                ExclusionClause.lblAdmin.Text = "Admin"
            End If
            ExclusionClause.MdiParent = frmMain
            ExclusionClause.lblExclusionClause_ID.Text = .CurrentRow.Cells(0).Value.ToString
            ExclusionClause.lblMember_Policy_ID.Text = .CurrentRow.Cells(1).Value.ToString
            ExclusionClause.lblGUID.Text = ""
            ExclusionClause.lblMemberID.Text = dgvPolicyDetails.CurrentRow.Cells(1).Value.ToString
            ExclusionClause.Show()
        End With
    End Sub
End Class